...
public class MyPageController {
	...
	public ModelAndView AdminUserManage(HttpSession session) {
		ModelAndView mav = new ModelAndView("mypage/frontpage");
		AdminUser AU = (AdminUser) session.getAttribute("User");
		mav.addObject("body", "../mypage/AdminStatics.jsp");
		if (AU.getAdmin_power() == 0) { // 총관리자, 모든 유저를 볼 수 있다.

		} else if (AU.getAdmin_power() == 1) { // 중고거래, 유저만 볼 수 있다.

		} else if (AU.getAdmin_power() == 2) { // 오픈, 업체,비회원,유저

		} else if (AU.getAdmin_power() == 3) {// 커뮤니티, 유저
		}
		return null; // 따로 페이지 둬서 뿌리는게 더 낫겠다.
	}
